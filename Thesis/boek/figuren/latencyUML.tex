...
\tikzstyle{every node}=[font=\scriptsize]
	\begin{package}[StreamPackage]{be.signalsync.sync}
		...
		\begin{interface}[text width=8cm]{SyncEventListener}{0,-10}
			\operation[0]{onSyncEvent(data : Map\textless StreamGroup, Double\textgreater) : void}
		\end{interface}
		
		\begin{abstractclass}[text width=4cm]{SyncStrategy}{-4,-7}
		\end{abstractclass}
		
		\begin{class}[text width=4cm]{StreamSetSlicer}{4,-7}
		\end{class}
		
		\begin{class}[text width=4cm]{StreamSet}{2.5,-9}
		\end{class}
				
		
		\coordinate [label={[align=center]\guillemotleft \textbf{bind}\guillemotright \\ T $\rightarrow$ Map\textless StreamGroup,float\lbrack \rbrack\textgreater}] (D) at (2.4,-2.0);
